...
## 🔄 Git Workflow (feature branches and commit discipline)

**All work happens on a feature branch — nothing is committed directly to `main`.**

### Branch per product domain

Before starting, update local refs and identify the product domain from the task/spec:

```bash
git fetch origin                      # refresh remote refs
git switch development                # integration parent
git pull --ff-only origin development
git branch --list 'feat/*'            # reuse an existing domain branch if one fits
git switch -c feat/ux-ui              # create it from development if missing
```

The canonical active domains are `ux-ui`, `mcp-system`, `system-prompt`,
`cli-commands`, `model-system`, `session-system`, `plans-mode`,
`context-compression`, `skills-system`, `agent-loop`, `tools-system` and
`project-rules`. Use `chore/quality-gates` for repository maintenance. Older
incident-oriented refs are historical and belong under `archive/legacy/`; they
are not starting points for new work.

| Prefix | Use |
|--------|-----|
| `feat/<domain>` | Product-domain work (e.g., `feat/ux-ui`, `feat/mcp-system`, `feat/cli-commands`) |
| `fix/<bug>` | Bug fixes |
...
- **Unrelated changes** found while working (a bug in another module, a stray file): don't commit them with the feature — report them, or handle them on their own branch/commit later.
- Leave the branch ready for review/merge into `development`; don't merge or delete it unless asked.
...
1. Create your feature branch (above).
...
5. Sync documentation, feature registry and CHANGELOG where applicable.
6. Open a **Pull Request** referencing the spec (e.g., `Refs: specs/2026-08-25-tui-overhaul`).
...
- [ ] Everything was committed on a feature branch, with only that feature's files (Rule 8)
