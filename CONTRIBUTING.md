...
Documentation has its own branch: `docs/documentation`. After implementing a
feature, switch to this branch before changing `README.md`, `CONTRIBUTING.md`,
anything under `docs/`, `features/` or `specs/`, `CHANGELOG.md`, or the
`.clinerules` process rules:

```bash
git switch docs/documentation
git pull --ff-only origin docs/documentation
# edit and validate documentation
git add README.md CHANGELOG.md       # stage only the exact documentation paths
git diff --cached --stat
git commit -m "docs(scope): synchronize documentation"
git push origin docs/documentation
```

The documentation branch is merged into `development` separately from the
product-code branch. Do not commit documentation files on `feat/*`, `fix/*`,
`refactor/*` or `chore/*` branches unless the file is generated by an
automated release process explicitly documented for that branch.
...
5. Switch to `docs/documentation` and sync documentation, feature registry and CHANGELOG there where applicable.
