...
| 2026-09-04 | Added interactive pending-confirmation modal (ADR-0006): when a turn produces `pending` records, a `select`-based modal opens before the prompt re-attaches, offering `Aceitar` / `Recusar` / `Aceitar todas` per record. `Aceitar todas` is a single atomic mutation; the user can disable the modal for the rest of the process (`skipConfirm`, per-process only). The existing `/memory accept <id>` and `/memory reject <id>` commands remain as the always-available escape hatch. | `specs/2026-09-04-memory-confirm-modal`, ADR-0006 |
