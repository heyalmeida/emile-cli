...
- **Interactive pending-confirmation modal** (`specs/2026-09-04-memory-confirm-modal`, ADR-0006): when a turn produces one or more `pending` records, a `select`-based modal opens before the prompt re-attaches, offering `Aceitar` / `Recusar` / `Aceitar todas` per record. `Aceitar todas` is a single atomic mutation through the same lock + WAL + atomic-rename path as `/memory accept <id>`, and sets a per-process `skipConfirm` flag so the modal does not re-open for the rest of the session. The existing `/memory accept <id>`, `/memory reject <id>`, `/forget` and `/memory clear` commands remain the always-available escape hatch; new `/memory confirm-on` and `/memory confirm-off` re-enable or disable the modal mid-session. Submitting a new prompt while the modal is open cancels it without changing the pending records.
...
### Fixed
- **Sandbox-safe config permission fixture:** subprocess assertions now execute Node directly and capture output through temporary file descriptors instead of requiring a nested shell or pipe, preserving the same isolated-home coverage in restricted test environments.
...
- **Cancel (Esc/Ctrl+C) now aborts the in-flight stream**: the persistent prompt never wired Esc/Ctrl+C to the turn control (the keys only terminated the REPL, leaving the stream running for minutes), and the agent loop only noticed a cancel when the next stream chunk arrived — a provider stalled in "thinking" produced no chunks, so the cancel never fired. Esc/Ctrl+C during a turn now call `control.requestStop()`, which also aborts the HTTP request through a real `AbortSignal` threaded into the OpenAI client (streaming and non-streaming); aborts are never retried and are reported as `Turn canceled`, not as stream errors.
- **Prompt stays usable across agent turns and `/switch`**: on submit, `listenTurnKeys` owns stdin but keeps the same full frame available while the agent works. It routes spinner/reasoning/output writes above the draft and leaves the real cursor at the typed text; async pickers such as `/switch` receive exclusive key ownership, after which the persistent prompt restores raw mode and resumes stdin.
- **Crash on first prompt render**: the persistent input field referenced an undeclared variable (`lastCursorLine`) in `src/ui/prompt-input-persistent.js`, throwing `ReferenceError` as soon as the prompt was drawn; the state variable is now properly initialized.
- **Cursor lag while typing**: the persistent prompt positioned the terminal cursor without accounting for the 2-space gutter of each block line AND the separator space between the prompt glyph and the typed text on the first input row, so the caret rendered 2 columns behind the typed text; the erase-up distance for the previous render was also miscalculated (leftover rows after redraws, e.g. with autocomplete matches visible).
- **Spinner no longer renders inside the prompt block**: `onSubmit` is async in the REPL (it awaits command dispatch before flagging the agent busy), so the post-submit redraw used to fire in that gap and the agent spinner — which writes at the current cursor — landed inside the freshly drawn input block. The redraw is now deferred until the submission settles (which also makes the `'cancel'` verdict from the async `onSubmit` actually terminate the REPL), and rendering stays suppressed while a submission is in flight.
- **Persistent prompt rebuilt** (grok-build/codex-style inline block): the renderer now uses one pure layout builder (`buildPromptLayout`) whose lines are clipped to terminal width, so displayed height and erase math cannot drift. Arrow keys navigate slash commands, Enter completes then submits, Tab accepts the highlighted completion (and toggles Plans mode only without a match), Esc clears an idle draft, and `/rewind` can refill the live field. The ANSI-terminal regression suite covers 60/80/120 columns, nested `/switch` ownership and post-picker typing.
- **Plan progress moved to the prompt footer**
- **Thinking/spinner diagnostic**
- **Empty-stream notice**: when the model returns a stream with no chunks at all (no `reasoning_content`, no `reasoning_details`, no `content`) and the turn was not cancelled or errored, the agent loop now writes a dim `· (empty response)` line where the spinner used to be. Without this, the user saw a blank line between the spinner and the prompt redraw on providers/models that occasionally return empty responses (e.g. `minimax-m3:free` via OpenRouter).
: a new opt-in `EMILE_DEBUG_THINKING=1` env var logs every `process.stdout.write` from `src/ui/spinner.js` and `src/ui/thinking.js` to `process.stderr` with the pre/post cursor row, to pinpoint the visual flicker between the `⠋ thinking…` spinner and the `✻ Thinking…` header during the active-turn prompt arbitration. Off by default.
: in plans mode the agent loop used to print `   Plan Progress: X/Y tasks completed` on every iteration, so the same line accumulated in the chat. The footer (idle and active-turn frames) now shows `tasks: X/Y` in `C.warn` while the plan is incomplete and `C.success` when it is done; the loop no longer writes that line. The segment is omitted when `task.md` is missing or has no checkboxes.
- **`feat/tools-system` aligned with `fix/persistent-prompt-input`** (`9d5b0d0`): the local branch tip had a partial GitHub Desktop auto-merge left over (`MERGE_HEAD` missing, `AUTO_MERGE` and conflict markers still in the index/working tree, three files reported as "deleted by us" though they were present on disk). The branch was reset to the `fix/persistent-prompt-input` tip from the local backup (`backup/fix/persistent-prompt-input`); no committed work is lost (orphaned commits remain reachable through the reflog and the object graph), and the seven conflicted files now reflect the shipped state with the four new commits on top.

### Added
- **Native user-global agent memory** (`specs/2026-09-03-global-agent-memory`): one provider- and MCP-independent store under `~/.emile/memory/v1/` now carries confirmed preferences, workflow conventions and recurring corrections across workspaces. It defaults to `ask`, supports two-session `auto` corroboration and explicit conflicts, injects only bounded transient context below current/project instructions, exposes `/memory`, `/remember`, `/forget` plus strict private proposal/recall tools, redacts memory content from session persistence, and provides WAL/backup recovery, token-owned writer locks, restrictive permissions, secret/identifier rejection, confirmed export and application-copy purging on forget/clear.
...
- **Session lifecycle hardening** (`specs/2026-09-02-session-lifecycle`):
  - **Ordered shutdown** (`src/lifecycle/`): SIGINT/SIGTERM/SIGHUP/`beforeExit` handlers run 6 phases in order (stop-input → drain-tools → flush-session → flush-memory → close-mcp → restore-terminal) with a global 3 s cap and `--verbose` per-phase timing.
...
### Security
- **Global-memory trust boundary:** memory reads use no-follow/non-blocking descriptors with regular-file verification, concurrent first-use writers serialize under owned lock tokens, and inferred evidence from quoted sources, one-off task scope, unstable instructions, secrets, private keys, identifiers or security-bypass requests is rejected before persistence.
